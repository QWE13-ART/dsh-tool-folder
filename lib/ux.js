...
/**
 * 轻目录：跨 server 轮转配额（name + 描述截 80 + server）。
 * limit<=0 → []（关闭回退，行为与旧版一致）。
 *
 * 2026-09-02（审计修复）：纯字典序 slice(0, limit) 会让名字靠后的 server
 * （mcp__windows 等）在截断后全部不可见 → 模型零匹配回退时误判"没有该能力"。
 * 改为：按 server 分组（组内字典序）→ round-robin 出队——小 server 必有代表、
 * 大 server 拿多数、同输入同输出（确定性）。server 函数为每工具独立名时行为
 * 与旧字典序一致（每组 1 个，轮转 = 字典序）。
...
export function fallbackCatalog(allTools, limit, serverOf) {
  ...
  for (const t of allTools) {
    const s = svc(t.name) || "(none)";
    if (!byServer.has(s)) byServer.set(s, []);
    byServer.get(s).push(t);
  }
  // Deterministic: server order by name, tools inside each server sorted by name.
  const queues = [...byServer.entries()]
    .sort((a, b) => String(a[0]).localeCompare(String(b[0])))
    .map(([, ts]) => ts.slice().sort((a, b) => String(a.name).localeCompare(String(b.name))));
  const out = [];
  while (out.length < lim && queues.some((q) => q.length > 0)) {
