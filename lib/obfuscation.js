...
const EXEC_VERB = /(?:iex|invoke-expression|invoke-command|iwr|irm|start-process|start-job|&|cmd\s*\/[cC]|sh\s+-c|bash\s+-c|python\s+-[cC]|node\s+-e|powershell|pwsh|mshta|rundll32|regsvr32|wscript|cscript)\b/i;
...
export function detectObfuscation(rawText, cleanText) {
  ...
  if (RE_B64_PAYLOAD.test(probe) && EXEC_VERB.test(probe)) {
...
/**
 * Combined check used by verdict(): returns the first obfuscation hit, or
 * null when the text looks clean.
 */
export function checkObfuscation(text) {
  const raw = String(text || "");
  if (!raw) return null;
  const clean = deobfuscate(raw);
  return detectObfuscation(raw, clean);
}
