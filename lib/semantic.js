...
let _timeout = EMBED_TIMEOUT_MS; // P3-2 (2026-09-02): configurable via configureSemantic

/**
 * Configure the endpoint (called by plugin apply with cfg.ollamaBase/model).
 * @param {{ollamaBase?: string, embedModel?: string, embedTimeoutMs?: number}} cfg
 */
export function configureSemantic(cfg = {}) {
  if (cfg && typeof cfg === "object") {
    ...
    if (Number.isFinite(cfg.embedTimeoutMs) && cfg.embedTimeoutMs >= 100) _timeout = cfg.embedTimeoutMs;
  }
}

/** One embedding vector for a text, or null on any failure. */
export async function embedText(text, timeoutMs = _timeout) {
 ...
 */
export async function buildSemanticIndex(docs, timeoutMs = _timeout, opts = {}) {
