...
export function isExecTool(name) {
  ...
  // All segments except the `mcp` marker participate — a `pwsh`/`db`/`vm`
  // server name is itself an exec signal, while `mcp__github__get_file_contents`
  // has none. Over-recognition is ~free (gate only); under-recognition is a bypass.
